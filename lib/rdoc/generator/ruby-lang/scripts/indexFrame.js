...
document.addEventListener('DOMContentLoaded', function () {
  ...
  setupResizing();
  restoreIndexesInfo();  // before search setup
  setupSearches();
  ...
  frameResized(true);  // initial resize
  ...
  for (const a of fileIndex.links)
    a.addEventListener('click', saveIndexesInfo);
  for (const a of classIndex.links)
    a.addEventListener('click', saveIndexesInfo);
  for (const a of methodIndex.links)
    a.addEventListener('click', saveIndexesInfo);

  function saveIndexesInfo(e) {
    if (fileIndex)
      window.parent.sessionStorage.setItem('fileIndex.scrollTop', fileIndex.list.scrollTop);
    window.parent.sessionStorage.setItem('classIndex.scrollTop', classIndex.list.scrollTop);
    window.parent.sessionStorage.setItem('classIndex.searchText', classIndex.searchBox.value);
    window.parent.sessionStorage.setItem('methodIndex.scrollTop', methodIndex.list.scrollTop);
    window.parent.sessionStorage.setItem('methodIndex.searchText', methodIndex.searchBox.value);
  }

  function restoreIndexesInfo() {
    // restore previous scroll positions & search states if any
    restoreIndexInfo('file', fileIndex);
    restoreIndexInfo('class', classIndex);
    restoreIndexInfo('method', methodIndex);
  }

  function restoreIndexInfo(name, index) {
    if (!index) return;
    const top = window.parent.sessionStorage.getItem(`${name}Index.scrollTop`);
    if (top)
      index.list.scrollTop = top;
    if (index.searchBox) {
      const searchText = window.parent.sessionStorage.getItem(`${name}Index.searchText`) || '';
      index.searchBox.value = searchText;
    }
  }
